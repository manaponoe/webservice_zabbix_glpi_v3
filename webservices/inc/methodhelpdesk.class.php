<?php
/**
 * @version $Id: methodhelpdesk.class.php 478 2019-12-12 15:26:07Z yllen $
 -------------------------------------------------------------------------
 LICENSE
 ...
 @copyright Copyright (c) 2009-2019 Webservices plugin team
...
class PluginWebservicesMethodHelpdesk extends PluginWebservicesMethodCommon {
   ...
   **/
   static function methodListHelpdeskItems($params, $protocol) {
         ...
         foreach ($CFG_GLPI["linkuser_types"] as $type) {
            if (($limit > 0)
               ...
               if (count($_SESSION['glpigroups'])
                  ...
                  $where = ['OR' => ['groups_id' => [implode(',', $_SESSION['glpigroups'])]]];
               }
               ...
               $where = $dbu->getEntitiesRestrictCriteria($table, '', '', $item->maybeRecursive());
      ...
      if ($type && ($type !='my')
         ...
         $where = $dbu->getEntitiesRestrictCriteria($table, '', $ent, $item->maybeRecursive());
         ...
         if (isset($params['name'])) {
            $where = ['name' => ['LIKE', addslashes($params['name'])]];
         }
   ...
   **/
   static function methodCreateTicket($params=[], $protocol) {
      ...
      if (isset($params['group'])) {
         $query['WHERE']['is_requester'] = 1;
      } else if (isset($params['groupassign'])) {
         $query['WHERE']['is_assig'] = 1;
      }
      $query['WHERE'] = $dbu->getEntitiesRestrictCriteria("glpi_groups",'',$data['entities_id'], true);
   ...
   **/
   static function methodGetTicket($params, $protocol) {
      ...
      foreach ($DB->request('glpi_items_tickets',
                            ['tickets_id' => $ticket->fields['id']]) as $datas) {
      ...
      if ($ticket->fields['slas_id_ttr']
          && $nextaction->getFromDBForTicket($ticket->fields['id'], SLM::TTR)) {
      ...
      if (Session::haveRightsOr('followup', [ITILFollowup::SEEPUBLIC,
                                             ITILFollowup::SEEPRIVATE])) {
         // Followups
         $query = ['FROM'  => 'glpi_itilfollowups',
                   'WHERE' => ['itemtype' => 'Ticket',
                               'items_id' => $params['ticket']],
                   'ORDER' => 'date DESC'];
   ...
   static function getTicketLinkedObjects($protocol, $params=[]) {
      ...
      if ($item->can(-1, READ)) {
         ...
         if ($item->maybePrivate()
             && ((($item == 'Followup')
                  && !Session::haveRightsOr('followup', [ITILFollowup::SEEPUBLIC,
                                                         ITILFollowup::SEEPRIVATE]))
                 || (($item == 'Task')
